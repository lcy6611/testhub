...
class ApiProject(models.Model):
    ...
    project = models.ForeignKey('projects.Project', on_delete=models.SET_NULL, null=True, blank=True, related_name='api_subprojects', verbose_name='关联主项目')
...
class ApiRequest(models.Model):
    ...
    collection = models.ForeignKey(ApiCollection, on_delete=models.CASCADE, related_name='requests',
                                   null=True, blank=True, verbose_name='所属集合')
    # 允许不关联集合时仍能归属到项目，用于“未分类接口”
    ...
    # 多对多关联项目：脚本可复用，一个脚本可被多个项目关联使用（对标性能测试 PerformanceScript.projects）
    projects = models.ManyToManyField(
        'ApiProject',
        related_name='api_scripts',
        blank=True,
        verbose_name='关联项目',
    )
