...
def collect_target_metrics(
    ...
    for key, meta in metric_items:
        ...
        timeline = [{"t": int(round(v[0] - start_ts)), "v": round(v[1], 3)} for v in raw]
        vals = [v[1] for v in raw]
...
def collect_execution_monitoring(execution) -> int:
    ...
    if not start or not stop:
        logger.info("执行 %s 缺少时间窗，跳过监控采集", getattr(execution, "execution_id", "?"))
        return 0
    if stop <= start:
        stop = start
    ...
    logger.info("执行 %s 采集监控指标 %d 条", getattr(execution, "execution_id", "?"), len(objs))
    return len(objs)
