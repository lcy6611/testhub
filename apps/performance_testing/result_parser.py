...
def _build_timeline(rows: List[Dict[str, Any]], bucket_s: Optional[int] = None) -> List[Dict[str, Any]]:
    if not rows:
        return []
    ts_min = min(r["ts"] for r in rows)
    ...
    span_s = max((ts_max - ts_min) / 1000.0, 0.001)
    if bucket_s is None:
        # 自适应：短测试（< 10s）用 1s 桶，否则按 span/8 取整（保证约 5~10 个桶），
        # 最大不超过 60s 桶。短测试也能画出有意义的曲线。
        bucket_s = max(1, min(60, int(span_s / 8) or 1))
