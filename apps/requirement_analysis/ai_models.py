...
class AIModelService:
    ...
    async def call_openai_compatible_api(config: AIModelConfig, messages: List[Dict[str, str]], *, meta: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        # 如果用户没有输入完整的chat/completions路径，尝试智能补全
        if not base_url.endswith('/chat/completions'):
            # 如果base_url已经以/v1结尾，直接拼接chat/completions
            if base_url.endswith('/v1'):
                url = f"{base_url}/chat/completions"
            # 如果base_url以/api结尾，可能不需要/v1，直接拼接chat/completions
            elif base_url.endswith('/api'):
                url = f"{base_url}/chat/completions"
            # 如果base_url包含/v1/，说明已经包含版本路径，直接拼接chat/completions
            elif '/v1/' in base_url:
                url = f"{base_url}/chat/completions"
            # 否则，默认添加 v1/chat/completions（标准OpenAI格式）
            else:
