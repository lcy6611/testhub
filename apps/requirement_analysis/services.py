...
class DocumentProcessor:
    ...
    def extract_images_from_docx(file_path: str) -> List[Dict[str, Any]]:
        """
        从 docx 提取内嵌图片，返回 [{content_type, bytes}] 列表。
        说明：
        - 这里不保证顺序严格对应文档出现顺序，但足够用于“图文一起理解”的多模态输入。
        - 若未安装 python-docx 或 docx 解析失败，返回空列表。
        """
        if docx is None:
            return []
        try:
            ...
            rels = getattr(doc.part, "_rels", None) or {}
            for rel in rels.values():
                try:
                    ...
                    if blob and ("image" in (content_type or "") or "image" in target_ref):
                        images.append({"content_type": content_type or "image/png", "bytes": blob})
                except Exception:
                    continue

            return images
        except Exception as e:
            logger.error(f"Word文档图片提取失败: {e}")
            return []

    @classmethod
    def _compress_image_bytes(cls, raw: bytes, content_type: str) -> Optional[Dict[str, Any]]:
        """
        将图片压缩/缩放为 JPEG，返回 {content_type, bytes}。
        若 Pillow 不可用或图片不可读，返回 None。
        """
        if Image is None:
            return None
        try:
            with Image.open(io.BytesIO(raw)) as im:
                ...
                w, h = im.size
                max_edge = max(w, h)
                if max_edge > cls.MAX_IMAGE_EDGE:
                    scale = cls.MAX_IMAGE_EDGE / float(max_edge)
                    new_size = (max(1, int(w * scale)), max(1, int(h * scale)))
                    im = im.resize(new_size)
                out = io.BytesIO()
                im.save(out, format="JPEG", quality=cls.JPEG_QUALITY, optimize=True)
                return {"content_type": "image/jpeg", "bytes": out.getvalue()}
        except Exception:
            return None

    @classmethod
    def extract_image_data_urls_from_docx(cls, file_path: str) -> List[str]:
        """
        从 docx 提取图片并转为 data URL（base64），用于多模态 messages。
        """
        imgs = cls.extract_images_from_docx(file_path)
        if not imgs:
            return []

        urls: List[str] = []
        for img in imgs[: cls.MAX_IMAGES]:
            content_type = img.get("content_type") or "image/png"
            raw = img.get("bytes") or b""
            if not raw:
                continue

            compressed = cls._compress_image_bytes(raw, content_type)
            if compressed:
    ...
    @classmethod
    def extract_image_data_urls(cls, document: RequirementDocument) -> List[str]:
        """根据文档类型提取图片 data URLs（docx 内嵌图或独立图片文件）。"""
        try:
            if document.document_type == "docx":
                return cls.extract_image_data_urls_from_docx(document.file.path)
            if document.document_type == "image":
                with open(document.file.path, "rb") as fp:
                    return cls.bytes_to_data_urls([fp.read()], ["image/jpeg"])
            return []
        except Exception:
            return []

    @classmethod
    def bytes_to_data_urls(cls, raw_list: List[bytes], content_types: Optional[List[str]] = None) -> List[str]:
        """将原始图片字节转为 data URL 列表（压缩后）。"""
        urls: List[str] = []
        content_types = content_types or []
        for idx, raw in enumerate(raw_list[: cls.MAX_IMAGES]):
            if not raw:
                continue
            content_type = content_types[idx] if idx < len(content_types) else "image/png"
            compressed = cls._compress_image_bytes(raw, content_type)
            if compressed:
                content_type = compressed["content_type"]
                raw = compressed["bytes"]
    ...
    @classmethod
    def upload_files_to_data_urls(cls, files) -> List[str]:
        """上传的图片文件转为 data URLs。"""
        raw_list: List[bytes] = []
        types: List[str] = []
        for f in files or []:
            if not f:
                continue
            mime = getattr(f, "content_type", "") or "image/png"
            if not mime.startswith("image/"):
                continue
            raw_list.append(f.read())
            types.append(mime)
            if len(raw_list) >= cls.MAX_IMAGES:
                break
        return cls.bytes_to_data_urls(raw_list, types)
