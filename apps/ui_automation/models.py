...
class UiProject(models.Model):
    ...
    project = models.ForeignKey('projects.Project', on_delete=models.SET_NULL, null=True, blank=True, related_name='ui_subprojects', verbose_name='关联主项目')
...
class AISuite(models.Model):
    """AI 测试套件（仅包含 AI 用例，无脚本）"""
    EXECUTION_STATUS_CHOICES = [
        ('not_run', '未执行'),
        ('passed', '通过'),
        ('failed', '失败'),
    ...
    project = models.ForeignKey(UiProject, on_delete=models.CASCADE, related_name='ai_suites', verbose_name='关联项目')
    name = models.CharField(max_length=200, verbose_name='套件名称')
    description = models.TextField(blank=True, verbose_name='套件描述')
...
class AISuiteCase(models.Model):
    """AI 套件与 AI 用例的关联（仅用例，无脚本）"""
    ai_suite = models.ForeignKey(AISuite, on_delete=models.CASCADE, related_name='suite_cases', verbose_name='AI套件')
    ai_case = models.ForeignKey(AICase, on_delete=models.CASCADE, verbose_name='AI用例')
    order = models.IntegerField(default=0, verbose_name='执行顺序')
...
class AIScheduledTask(models.Model):
    ...
    name = models.CharField(max_length=200, verbose_name='任务名称')
    task_type = models.CharField(max_length=20, choices=TASK_TYPE_CHOICES, verbose_name='任务类型')
    trigger_type = models.CharField(max_length=20, choices=TRIGGER_TYPE_CHOICES, default='CRON', verbose_name='触发类型')
    cron_expression = models.CharField(max_length=100, blank=True, verbose_name='Cron表达式')
    interval_seconds = models.IntegerField(null=True, blank=True, verbose_name='间隔秒数')
    execute_at = models.DateTimeField(null=True, blank=True, verbose_name='单次执行时间')
    project = models.ForeignKey(UiProject, on_delete=models.CASCADE, verbose_name='关联项目')
    ai_suite = models.ForeignKey(
        AISuite, on_delete=models.CASCADE, null=True, blank=True, related_name='scheduled_tasks', verbose_name='AI套件'
    )
    ai_case = models.ForeignKey(
        AICase, on_delete=models.CASCADE, null=True, blank=True, related_name='scheduled_tasks', verbose_name='AI用例'
    )
    execution_mode = models.CharField(max_length=20, default='text', verbose_name='执行模式')
    headless = models.BooleanField(default=False, verbose_name='无头模式')
    notify_on_success = models.BooleanField(default=False, verbose_name='成功时通知')
    notify_on_failure = models.BooleanField(default=False, verbose_name='失败时通知')
    notification_type = models.CharField(max_length=20, blank=True, verbose_name='通知类型')
    notify_emails = models.JSONField(default=list, blank=True, verbose_name='通知邮箱列表')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='ACTIVE', verbose_name='任务状态')
    last_run_time = models.DateTimeField(null=True, blank=True, verbose_name='最后运行时间')
    next_run_time = models.DateTimeField(null=True, blank=True, verbose_name='下次运行时间')
...
class UiScriptGeneration(models.Model):
    """用例 → UI 自动化脚本 生成任务（增量功能，独立、可整体删除）。

    记录一次从 testcases.TestCase 自动生成 UI 自动化脚本的任务：
    智能体按用例在真实系统中抓取元素 → 生成 Playwright 脚本 → 回放自验证。
    source_testcase_id 存 testcases.TestCase 主键（跨 app 不建 FK，便于整体回退）。
    """
    STATUS_CHOICES = [
        ('pending', '等待中'),
        ('running', '执行中'),
    ...
    generated_script = models.ForeignKey(
        TestScript, null=True, blank=True,
        on_delete=models.SET_NULL, related_name='+', verbose_name='生成的脚本（兼容旧数据）'
    )
    generated_test_case = models.ForeignKey(
        'TestCase', null=True, blank=True,
        on_delete=models.SET_NULL, related_name='+', verbose_name='生成的UI用例'
    )
    ai_execution_record = models.ForeignKey(
        'AIExecutionRecord', null=True, blank=True,
        on_delete=models.SET_NULL, related_name='+', verbose_name='智能体执行记录'
    )
    ui_suite = models.ForeignKey(
        TestSuite, null=True, blank=True,
        on_delete=models.SET_NULL, related_name='+', verbose_name='生成的套件'
    )
    suite_execution = models.ForeignKey(
        'TestExecution', null=True, blank=True,
        on_delete=models.SET_NULL, related_name='+', verbose_name='套件执行记录'
    )
    suite_exec_status = models.CharField(max_length=20, default='', blank=True, verbose_name='套件执行状态')
    suite_exec_pass_rate = models.FloatField(default=0, verbose_name='套件执行通过率')
    ...
    # Playwright 代码单独存放（不入用例 description，避免列表卡片被代码撑乱；仅在结果页显示）
    playwright_code = models.TextField(blank=True, default='', verbose_name='Playwright 代码（仅参考）')
    # 重试配置（增量功能）：retry_limit 为失败后自动重试次数（可配置，默认 1）；retry_count 为已重试次数
    retry_limit = models.PositiveSmallIntegerField(default=1, verbose_name='失败自动重试次数')
    retry_count = models.PositiveSmallIntegerField(default=0, verbose_name='已重试次数')
    ...
    def __str__(self):
        return f"#{self.id} {self.source_testcase_title} -> {self.status}"
