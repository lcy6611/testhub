...
class PlaywrightTestEngine:
    ...
    async def navigate(self, url: str) -> Tuple[bool, str]:
        ...
        """
        try:
            # 1. 规范化 URL，自动补全协议，避免 "Unsupported URL" 问题
            normalized_url = (url or "").strip()
            if not normalized_url:
                return False, "✗ 导航失败: URL 为空，请在项目中配置基础地址（Base URL）"

            if not normalized_url.startswith(("http://", "https://", "file://")):
                normalized_url = "http://" + normalized_url

            # 使用 load 等待页面加载完成，并将超时时间提高到 60 秒
            # 之前使用 networkidle 且 30 秒超时，容易在页面有长连接/轮询时误判为失败
            await self.page.goto(normalized_url, wait_until='load', timeout=60000)

            # 额外等待 2 秒，确保前端 SPA 框架完成首屏渲染
            await asyncio.sleep(2)

            log = f"✓ 成功导航到: {normalized_url}\n"
            log += "  - 等待页面加载完成（load + 额外2秒）"
            return True, log
        except Exception as e:
            log = f"✗ 导航失败: {url}\n  - 规范化后: {normalized_url if 'normalized_url' in locals() else url}\n  - 错误: {str(e)}"
            return False, log
