...
class AIExecutionReportGenerator:
    ...
    def generate_summary_report(self) -> Dict[str, Any]:
        ...
        # 构建报告数据（含 detailed_steps 供 GIF 回放展示每步中文描述）
        steps_completed = self.record.steps_completed or []
        ...
        report = {
            ...
            'steps': step_info,
            'detailed_steps': detailed_steps,  # 每步自然语言描述，供 GIF 回放使用
    ...
    def _parse_execution_logs(self, logs: str) -> Dict[str, Any]:
        ...
        # 先按 Step 块切分
        step_block_pattern = r'\[Step (\d+)\](.*?)(?=\n\[Step \d+\]|\Z)'
        blocks = re.findall(step_block_pattern, logs, re.S)

        for step_num, block in blocks:
            step_number = int(step_num)

            # 在块内优先寻找“执行动作:”这一行
            action_line_match = re.search(r'执行动作:\s*(.+)', block)
            if action_line_match:
                action_str = action_line_match.group(1).strip()
            else:
                # 兼容旧格式“执行: ...”
                legacy_match = re.search(r'执行:\s*(.+)', block)
                action_str = legacy_match.group(1).strip() if legacy_match else ''

            if action_str:
                total_actions += len(action_str.split(' | '))
    ...
    def _parse_detailed_steps(self, logs: str, steps_completed: List) -> List[Dict[str, Any]]:
        ...
        # 先从 steps_completed 中提取信息，优先使用自然语言描述（description）作为操作展示
        for step in steps_completed:
            detailed_steps.append({
                'step_number': step.get('step_number', step.get('step', 0)),
                'action': step.get('description') or step.get('action', ''),
                'element': step.get('element', ''),
                'status': step.get('status', 'completed'),
                'timestamp': step.get('timestamp', ''),
                'thinking': step.get('thinking', ''),
                'evaluation': step.get('evaluation', ''),  # 验证信息（若有）
                'screenshot': step.get('screenshot', ''),
            })

        # 如果没有 steps_completed，从日志中解析（兼容新旧两种日志格式）
        if not detailed_steps and logs:
            step_block_pattern = r'\[Step (\d+)\](.*?)(?=\n\[Step \d+\]|\Z)'
            blocks = re.findall(step_block_pattern, logs, re.S)
            ...
            for step_num, block in blocks:
                ...
                # 提取思考 / 验证 / 执行动作，尽量还原成自然语言步骤
                thinking = ''
                eval_prev = ''
                action_str = ''

                thinking_match = re.search(r'思考\(thinking\):\s*(.+)', block)
                if thinking_match:
                    thinking = thinking_match.group(1).strip()

                eval_match = re.search(r'验证\(evaluation_previous_goal\):\s*(.+)', block)
                if eval_match:
                    eval_prev = eval_match.group(1).strip()
                ...
                if action_line_match:
                    ...
                    legacy_match = re.search(r'执行:\s*(.+)', block)
                    if legacy_match:
                        action_str = legacy_match.group(1).strip()

                # 组装一个尽量可读的 action 描述
                if not action_str and (thinking or eval_prev):
                    # 没有明确的动作，就用思考 / 验证内容作为描述
                    action_desc = thinking or eval_prev
                else:
                ...
                detailed_steps.append({
                    ...
                    'action': action_desc,
                    'thinking': thinking,
                    'evaluation': eval_prev,
                    'status': 'completed',
                    'timestamp': '',
                })

        # 如果既有 steps_completed 又有日志，则尝试根据日志为每步补充 evaluation
        if logs and detailed_steps:
            step_block_pattern = r'\[Step (\d+)\](.*?)(?=\n\[Step \d+\]|\Z)'
            blocks = re.findall(step_block_pattern, logs, re.S)
            eval_map = {}
            for step_num, block in blocks:
                eval_match = re.search(r'验证\(evaluation_previous_goal\):\s*(.+)', block)
                if eval_match:
                    eval_map[int(step_num)] = eval_match.group(1).strip()

            if eval_map:
                for step in detailed_steps:
                    try:
                        num = int(step.get('step_number') or step.get('step') or 0)
                    except Exception:
                        num = 0
                    if num in eval_map and not step.get('evaluation'):
                        step['evaluation'] = eval_map[num]
    ...
    def _analyze_step_performance(self, steps_completed: List) -> List[Dict[str, Any]]:
        ...
        # 收集所有步骤（优先从 steps_completed，否则从日志解析，兼容新旧日志格式）
        all_steps = []
        if steps_completed:
            for i, step in enumerate(steps_completed):
                action_desc = step.get('action', '') or step.get('description', '')
                if not action_desc:
                    thinking = step.get('thinking', '')
                    element = step.get('element', '')
        ...
        # 基于操作复杂度分配权重（兼容文本模式 点击[N]、视觉模式自然语言描述）
        def get_step_complexity(action_str):
            ...
            # 导航类操作权重最高（页面加载）
            if '访问:' in s or 'go_to_url' in s or 'navigate' in s or s.startswith('导航'):
                return 2.0

            # 输入操作次之
            if "输入:" in s or 'input_text' in s or '输入 ' in s or (s.startswith('输入') and len(s) > 2):
                return 1.5

            # 切换标签
            if '切换标签' in s or 'switch_tab' in s:
                return 1.2

            # 新标签打开
            if '新标签打开' in s or 'open_new_tab' in s:
                return 1.8

            # 点击操作（文本 点击[N] / 视觉 在 (x,y) 处点击）
            if '点击[' in s or 'click_element' in s or '处点击' in s or (s.startswith('点击') and len(s) > 2):
                return 1.0
