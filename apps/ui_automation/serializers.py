...
class UiNotificationConfigSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'webhook_bots', 'is_default', 'is_active',
...
class TestSuiteSerializer(serializers.ModelSerializer):
    ...
    script_names = serializers.SerializerMethodField()
    ...
    def get_script_names(self, obj):
        """获取脚本名称列表（用于列表展示，最多返回前10个）"""
        try:
            qs = obj.suite_scripts.select_related('test_script').order_by('order')
            names = []
            for ss in qs[:10]:
                if ss.test_script_id:
                    names.append(ss.test_script.name)
            return names
        except Exception:
            return []
...
class TestExecutionSerializer(serializers.ModelSerializer):
    ...
    def get_test_case_name(self, obj):
        """用例/脚本名称：脚本用脚本名，用例用 result_data 或占位"""
        if obj.test_script_id:
            return obj.test_script.name if obj.test_script else '-'
        rd = obj.result_data or {}
        cases = rd.get('test_cases') or []
        if cases:
            return cases[0].get('test_case_name') or '-'
        return '-'
...
class TestCaseSerializer(serializers.ModelSerializer):
    ...
    used_in_suites_count = serializers.SerializerMethodField()
    ...
    def get_used_in_suites_count(self, obj):
        try:
            from .models import TestSuiteTestCase
            return TestSuiteTestCase.objects.filter(test_case_id=obj.id).values('test_suite_id').distinct().count()
        except Exception:
            return 0
...
class AIExecutionRecordSerializer(serializers.ModelSerializer):
    ...
    project_name = serializers.CharField(source='project.name', read_only=True)
    ai_suite_id = serializers.IntegerField(read_only=True, required=False)
    ai_suite_name = serializers.CharField(source='ai_suite.name', read_only=True, required=False)
    ...
    suite_run_id = serializers.CharField(read_only=True, required=False)

    class Meta:
        model = AIExecutionRecord
        fields = [
            'id', 'project', 'project_id', 'project_name',
            'ai_suite_id', 'ai_suite_name', 'suite_run_id',
            'ai_case', 'ai_case_id', 'ai_case_name', 'case_name',
            'task_description',
...
class AISuiteCaseSerializer(serializers.ModelSerializer):
    ai_case_name = serializers.CharField(source='ai_case.name', read_only=True)
    ai_case_task_description = serializers.CharField(source='ai_case.task_description', read_only=True)

    class Meta:
        model = AISuiteCase
        fields = ('id', 'ai_suite', 'ai_case', 'ai_case_name', 'ai_case_task_description', 'order')


class AISuiteSerializer(serializers.ModelSerializer):
    suite_cases = AISuiteCaseSerializer(many=True, read_only=True)
    ...
    class Meta:
        model = AISuite
        fields = (
            'id', 'project', 'name', 'description', 'execution_status', 'execution_status_display',
            'passed_count', 'failed_count', 'suite_cases', 'case_count',
            'created_at', 'updated_at', 'created_by'
        )
        read_only_fields = ('execution_status', 'passed_count', 'failed_count', 'created_at', 'updated_at')

    def get_case_count(self, obj):
        return obj.suite_cases.count()


class AISuiteCreateUpdateSerializer(serializers.ModelSerializer):
    class Meta:
        model = AISuite
        fields = ('id', 'project', 'name', 'description')

    def create(self, validated_data):
        suite = AISuite.objects.create(**validated_data)
        case_ids = self.initial_data.get('ai_cases', [])
        for i, cid in enumerate(case_ids):
            AISuiteCase.objects.get_or_create(ai_suite=suite, ai_case_id=cid, defaults={'order': i})
        return suite
    ...
    def update(self, instance, validated_data):
        ...
        instance.save()
        case_ids = self.initial_data.get('ai_cases', None)
        if case_ids is not None:
            instance.suite_cases.all().delete()
            for i, cid in enumerate(case_ids):
                AISuiteCase.objects.create(ai_suite=instance, ai_case_id=cid, order=i)
        return instance


class AIScheduledTaskSerializer(serializers.ModelSerializer):
    project_name = serializers.CharField(source='project.name', read_only=True)
    ai_suite_name = serializers.CharField(source='ai_suite.name', read_only=True)
    ai_case_name = serializers.CharField(source='ai_case.name', read_only=True)
    task_type_display = serializers.CharField(source='get_task_type_display', read_only=True)
    status_display = serializers.CharField(source='get_status_display', read_only=True)
    ...
    class Meta:
        ...
        fields = (
            'id', 'name', 'task_type', 'task_type_display', 'trigger_type', 'cron_expression',
            'interval_seconds', 'execute_at', 'project', 'project_name', 'ai_suite', 'ai_suite_name',
            'ai_case', 'ai_case_name', 'execution_mode', 'headless',
            'notify_on_success', 'notify_on_failure', 'notification_type', 'notify_emails',
            'status', 'status_display', 'last_run_time', 'next_run_time',
            'total_runs', 'successful_runs', 'failed_runs', 'last_result', 'error_message',
            'created_at', 'updated_at', 'created_by'
        )
        read_only_fields = (
            'last_run_time', 'next_run_time', 'total_runs', 'successful_runs', 'failed_runs',
            'last_result', 'error_message', 'created_at', 'updated_at', 'created_by'
        )

    def create(self, validated_data):
        task = AIScheduledTask.objects.create(**validated_data)
        try:
            task.next_run_time = task.calculate_next_run()
            task.save(update_fields=['next_run_time'])
        except Exception as e:
            raise serializers.ValidationError({
                'cron_expression': [f'Cron 表达式无效或无法计算下次执行时间: {str(e)}']
            })
        return task
...
class AiNotificationLogSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            'id', 'task', 'task_name', 'task_type', 'task_type_display',
            'notification_type', 'notification_type_display', 'actual_notification_type_display',
            'sender_name', 'sender_email', 'recipient_info', 'webhook_bot_info', 'notification_content',
            'status', 'status_display', 'error_message', 'response_info', 'created_at'
        )
        read_only_fields = ['created_at']
    ...
    def get_task_type_display(self, obj):
        if not obj.task_type:
            return '未记录'
        return {'AI_SUITE': 'AI套件执行', 'AI_CASE': 'AI用例执行'}.get(obj.task_type, obj.task_type)

    def get_actual_notification_type_display(self, obj):
        """与 UI 自动化通知类型显示一致：优先 webhook 机器人，再邮箱，再回退到任务配置或 '-'"""
        if obj.webhook_bot_info:
            bot_type = obj.webhook_bot_info.get('bot_type', '') or obj.webhook_bot_info.get('type', '')
            bot_name = obj.webhook_bot_info.get('bot_name', '') or obj.webhook_bot_info.get('name', '')
            type_map = {
                'wechat': '企微机器人',
                'feishu': '飞书机器人',
                'dingtalk': '钉钉机器人'
        ...
        if obj.recipient_info:
            if isinstance(obj.recipient_info, list) and len(obj.recipient_info) > 0:
                return '邮箱通知'
            if isinstance(obj.recipient_info, dict) and obj.recipient_info.get('email'):
                return '邮箱通知'

        if obj.task:
            notification_type = getattr(obj.task, 'notification_type', '') or ''
            type_map = {'email': '邮箱通知', 'webhook': 'Webhook机器人', 'both': '两种都发送'}
            return type_map.get(notification_type, notification_type) or '-'
        return '-'
