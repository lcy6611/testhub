...
def _to_bool(value, default=False):
    """将请求参数安全转为 bool（避免字符串 'false' 被当作 True）。"""
    if value is None:
        return default
...
def _resolve_headless_for_env(headless_requested):
    """无图形环境（如 Docker/Linux 无 DISPLAY）时强制使用无头，避免有头模式启动失败。"""
    headless_requested = _to_bool(headless_requested, default=False)
...
def extract_step_info(s, step_index):
    """提取步骤信息的辅助函数，返回结构化动作 + 自然语言描述。"""
    step_info = {'step': step_index}

    # 1. 原始动作信息（结构化），尽量完整保留到 raw_action
    raw_action = None
    ...
    if hasattr(s, 'action'):
        ...
        if isinstance(action_data, str):
            ...
            attrs = {}
            for key in ['type', 'description', 'goal', 'coordinate', 'text', 'output', 'result', 'url', 'value']:
        ...
        if callable(s):
            raw_action = f"<Action: {getattr(s, '__name__', 'unknown action')}>"
        else:
            raw_action = str(s)

    # 2. 基于 raw_action 生成自然语言描述，保证报告里有“人话步骤”
    description = ""
    if isinstance(raw_action, dict):
        # 优先使用模型自带的 description 字段
        description = (raw_action.get('description') or "").strip()
        action_type = (raw_action.get('type') or raw_action.get('action') or "").lower()

        if not description:
            if action_type == 'navigate':
                url = (raw_action.get('url') or raw_action.get('text') or "").strip()
                description = f"访问页面: {url}" if url else "访问目标页面"
            elif action_type in ('type', 'input_text', 'input'):
                text = (raw_action.get('text') or raw_action.get('value') or "").strip()
                description = f"在输入框输入: {text}" if text else "在输入框输入文本"
            elif action_type == 'click':
                target = (raw_action.get('goal') or raw_action.get('output') or "").strip()
                description = f"点击元素: {target}" if target else "点击页面元素"
            else:
                description = action_type or ""
    elif isinstance(raw_action, str):
        description = raw_action.strip()

    # 3. 输出给前端 / 报告：action 为可读描述，raw_action 保存原始结构
    step_info['raw_action'] = raw_action
    step_info['action'] = description or (str(raw_action) if raw_action is not None else "")
...
class UiProjectViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        auto_sync_project_create(instance, 'ui_automation')
    ...
    def perform_update(self, serializer):
        ...
        auto_sync_project_update(instance, 'ui_automation')
    ...
    def perform_destroy(self, instance):
        ...
        auto_sync_project_delete(instance, 'ui_automation')
        instance.delete()
...
class ElementGroupViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # 取消用户隔离：所有登录用户可见全部元素分组
        return ElementGroup.objects.all().select_related('project', 'parent_group').order_by('order', 'name')
...
class TestSuiteViewSet(viewsets.ModelViewSet):
    ...
    def run_suite(self, request, pk=None):
        ...
        # 传统模式执行（Playwright/Selenium）：套件可包含用例、脚本或两者，至少有一种即可执行
        ...
        try:
            # 在后台线程中执行测试
            import threading
            from .test_executor import TestExecutor, SUITE_STOP_SIGNALS

            SUITE_STOP_SIGNALS[test_suite.id] = False  # 启动前清除停止信号
    ...
    def stop_suite(self, request, pk=None):
        """停止正在执行的套件（设置停止信号，执行器在下一个用例前会退出并更新套件状态）"""
        test_suite = self.get_object()
        from .test_executor import SUITE_STOP_SIGNALS

        if test_suite.execution_status != 'running':
            return Response(
                {'message': '该套件未在执行中，无需停止'},
                status=status.HTTP_400_BAD_REQUEST
            )
        SUITE_STOP_SIGNALS[test_suite.id] = True
        return Response({'message': '已发送停止信号，套件将在当前用例结束后停止'})
...
class TestCaseViewSet(viewsets.ModelViewSet):
    ...
    def run(self, request, pk=None):
        ...
        try:
            ...
            if engine_type == 'selenium':
                ...
                if not is_available:
                    ...
                    try:
                        ...
                        safe_writeback_ui_test_case_execution(execution)
                    except Exception:
                        logger.warning("知识图谱回写 UI 执行结果失败", exc_info=True)
            ...
            headless_mode = _resolve_headless_for_env(headless_requested)
            if headless_mode and not headless_requested:
                execution_logs.append("检测到无图形界面环境（未设置 DISPLAY/WAYLAND_DISPLAY），已自动切换为无头模式以便执行。")
            ...
            if engine_type == 'selenium':
                ...
                def run_test_selenium():
                    """使用Selenium执行测试"""
                    browser_type = request.data.get('browser', 'chrome')
                    # 使用已根据环境解析过的 headless_mode（容器下已自动降级为无头）
                    ...
                    try:
                        ...
                        try:
                            ...
                            # 浏览器启动失败：写入 step_results 以便前端“执行日志”有内容
                ...
                # 在独立线程中运行Selenium测试（线程内未捕获异常需写回 execution_result / step_results）
                def run_test_selenium_safe():
                    try:
                        run_test_selenium()
                    except Exception as e:
                        import traceback
                        ...
                        execution_logs.append(f"✗ {str(e)}")
                        execution_logs.append("")
                        execution_logs.append(traceback.format_exc())
                        step_results.append({
                            'step_number': 0,
                            'action_type': '系统',
                            'description': '执行异常',
                ...
                import threading
                test_thread = threading.Thread(target=run_test_selenium_safe)
                test_thread.start()
                test_thread.join()
                ...
                # 在独立线程中运行Playwright测试（线程内异常需写回 execution_result / step_results，否则会误报成功且无日志）
                def run_test_in_thread_safe():
                    try:
                        run_test_in_thread()
                    except Exception as e:
                        import traceback
                ...
                import threading
                test_thread = threading.Thread(target=run_test_in_thread_safe)
                test_thread.start()
            ...
            # 保存实际使用的无头模式（容器下可能已从有头降级为无头）
            execution.headless = headless_mode
    ...
    @action(detail=False, methods=['post'], url_path='batch-delete-testcases')
    def batch_delete_testcases(self, request):
        """批量删除测试用例（连同步骤）。"""
        ids = request.data.get('ids', [])
        if not ids or not isinstance(ids, list):
            return Response({'error': '未提供要删除的用例ID列表'}, status=status.HTTP_400_BAD_REQUEST)
        ids = [int(i) for i in ids if str(i).isdigit()]
        if not ids:
            return Response({'error': 'ids 参数无效'}, status=status.HTTP_400_BAD_REQUEST)
        queryset = self.get_queryset().filter(id__in=ids)
        names = list(queryset.values_list('name', 'id'))
        deleted_count, _ = queryset.delete()
        for name, tid in names:
            try:
                log_operation('delete', 'test_case', tid, name, request.user)
            except Exception:
                pass
        return Response({'message': f'成功删除 {deleted_count} 条用例（含步骤）'})

    @action(detail=True, methods=['get'], url_path='used-in-suites')
    def used_in_suites(self, request, pk=None):
        """查询用例被哪些套件引用——支撑「可复用」展示。"""
        tc = self.get_object()
        rows = TestSuiteTestCase.objects.filter(test_case=tc).select_related('test_suite')
        suites = []
        seen = set()
        for row in rows:
            s = row.test_suite
            if s.id in seen:
                continue
            seen.add(s.id)
            suites.append({'id': s.id, 'name': s.name, 'status': s.status})
        return Response({'suites': suites, 'count': len(suites)})

    @action(detail=True, methods=['post'], url_path='clone-to-project')
    def clone_to_project(self, request, pk=None):
        """跨项目复制用例（步骤 + 元素关联）。目标项目若存在相同 locator 的 Element 则复用。"""
        target_project_id = request.data.get('project_id')
        if not target_project_id:
            return Response({'error': '未提供 project_id'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            target_project = UiProject.objects.get(id=int(target_project_id))
        except Exception:
            return Response({'error': '目标项目不存在'}, status=status.HTTP_404_NOT_FOUND)
        src = self.get_object()
        steps = list(src.steps.all().select_related('element', 'element__locator_strategy').order_by('step_number'))

        # 在目标项目下按 (strategy, value) 复用已有 Element
        target_elements = Element.objects.filter(project=target_project).select_related('locator_strategy')
        elem_map = {}
        for e in target_elements:
            key = (e.locator_strategy.name if e.locator_strategy else '', e.locator_value)
            elem_map[key] = e
        # 当前用例的元素的 (strategy, value) → 在目标项目找/建
        new_elem_by_id = {}
        for s in steps:
            if not s.element_id:
                continue
            es = s.element
            key = (es.locator_strategy.name if es.locator_strategy else '', es.locator_value)
            new_el = elem_map.get(key)
            if not new_el:
                new_el = Element.objects.create(
                    ...
                    locator_strategy=es.locator_strategy,
                    locator_value=es.locator_value,
                    backup_locators=es.backup_locators,
                    page=es.page,
                    validation_status='PENDING',
                    created_by=request.user if request.user.is_authenticated else None,
                )
                elem_map[key] = new_el
            new_elem_by_id[es.id] = new_el

        new_tc = TestCase.objects.create(
            project=target_project,
            name=f"{src.name}（复制自 #{src.id}·{src.project.name}）"[:200],
        ...
        for s in steps:
            TestCaseStep.objects.create(
                ...
                element=new_elem_by_id.get(s.element_id),
...
class AICaseViewSet(viewsets.ModelViewSet):
    ...
    def run(self, request, pk=None):
        ...
        支持两种模式：
        - 文本模型（默认）：复用 browser-use 文本代理
        - 视觉模型：走 vision_runner 独立视觉流水线，生成带中文字幕的 GIF
        """
        ai_case = self.get_object()

        execution_mode = (request.data.get('execution_mode') or 'text').strip().lower()
        # auto 在 AI 用例执行里等价于 text（保持文本模式逻辑不变）
        if execution_mode not in ('vision', 'text'):
            execution_mode = 'text'
        enable_gif = _to_bool(request.data.get('enable_gif', True), default=True)
        headless_requested = _to_bool(request.data.get('headless', False), default=False)
        headless = _resolve_headless_for_env(headless_requested)
        ...
        if headless and not headless_requested:
            execution_record.logs += "[System] 检测到无图形界面环境，已自动切换为无头模式以便执行。\n"
        ...
        def run_task():
            ...
            try:
                ...
                # 根据执行模式分流：视觉模式走 vision_runner，其它模式走文本流水线
                if execution_mode == 'vision':
                    history = run_vision_pipeline_sync(
                ...
                # 自动标记已完成的任务（仅在有 planned_tasks 时）
                if execution_record.planned_tasks:
                    AIExecutionRecordViewSet._auto_mark_completed_tasks(execution_record)
                ...
                # 无论成功/失败，只要有 history 都尝试处理 GIF，但不能影响原始状态
                try:
                    if history is not None:
                        AIExecutionRecordViewSet._process_gif_recording(execution_record, history)
                except Exception as gif_err:
                    logger.warning(f"⚠️ Failed to process GIF in finally block: {gif_err}")

                # 重新保存最终状态（包括可能更新的 gif_path）
                try:
                    execution_record.save()
                except Exception as save_err:
                    logger.warning(f"⚠️ Failed to save execution_record in finally: {save_err}")
                # 清理停止信号
...
class AIExecutionRecordViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return AIExecutionRecord.objects.all()
    ...
    def run_adhoc(self, request):
        """执行临时 AI 任务"""
        from .ai_agent import run_full_process_sync, _resolve_ai_execution_mode

        project_id = request.data.get('project_id')
        task_description = request.data.get('task_description')
        execution_mode_raw = request.data.get('execution_mode', 'text')
        execution_mode = _resolve_ai_execution_mode(execution_mode_raw)
        enable_gif = _to_bool(request.data.get('enable_gif', True), default=True)
        headless_requested = _to_bool(request.data.get('headless', False), default=False)
        headless = _resolve_headless_for_env(headless_requested)
        ...
        initial_logs = "正在分析任务...\n"
        if str(execution_mode_raw).strip().lower() == 'auto':
            initial_logs += f"[System] 自动模式已解析为: {execution_mode}\n"
        mode_label = '无头' if headless else '有头'
        initial_logs += f"[System] 浏览器模式: {mode_label}\n"
        if headless and not headless_requested:
            initial_logs += "[System] 检测到无图形界面环境，已自动切换为无头模式以便执行。\n"
        elif not headless and platform.system() == 'Linux' and os.getenv('DISPLAY'):
            initial_logs += (
                "[System] 有头模式运行在虚拟显示上（Docker 环境请通过 VNC 5900 端口查看浏览器）。\n"
            )
        ...
        def run_task():
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"⚠️ Failed to process GIF in finally block (adhoc): {gif_err}")

                try:
                    execution_record.save()
                except Exception as save_err:
                    logger.warning(f"⚠️ Failed to save execution_record in finally (adhoc): {save_err}")
    ...
    @action(detail=False, methods=['get'], url_path='report_summary')
    def report_summary(self, request):
        """
        AI 测试报告聚合接口（供前端“测试报告”列表使用）。
        当前实现：每条 AIExecutionRecord 视为一条报告行。
        """
        qs = self.get_queryset().order_by('-start_time')
        project_id = request.query_params.get('project')
        if project_id:
            qs = qs.filter(project_id=project_id)

        page = self.paginate_queryset(qs)
        records = page if page is not None else qs

        def build_row(r: AIExecutionRecord):
            duration = r.duration
            try:
            ...
            skipped = max(0, case_total - passed - failed)
            pass_rate = int(round((passed / case_total) * 100)) if case_total else 0
            return {
                'id': r.id,
                'suite_run_id': None,
                'suite_name': r.case_name or 'Adhoc Task',
                ...
                'pass_rate': pass_rate,
                'status': r.status,
                'execution_mode': getattr(r, 'execution_mode', None) or 'text',
                'duration': duration,
                'started_at': r.start_time,
                'executed_by_name': getattr(getattr(r, 'executed_by', None), 'username', None),
                'execution_ids': [r.id],
                'executions': [{
                    'id': r.id,
                    'case_name': r.case_name or 'Adhoc Task',
                    'execution_mode': getattr(r, 'execution_mode', None) or 'text',
                    'status': r.status,
                    'duration': duration,
        ...
        data = [build_row(r) for r in records]
        if page is not None:
    ...
    @staticmethod
    def _process_gif_recording(execution_record, history):
        """
        处理GIF录制文件
    ...
    @staticmethod
    def _auto_mark_completed_tasks(execution_record):
        """
        自动标记已完成的任务
...
class AISuiteViewSet(viewsets.ModelViewSet):
    """AI 测试套件视图集"""
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.OrderingFilter]
    filterset_fields = ['project']
    ...
    def get_queryset(self):
        return AISuite.objects.all()

    def get_serializer_class(self):
        if self.action in ('create', 'update', 'partial_update'):
            return AISuiteCreateUpdateSerializer
        return AISuiteSerializer
    ...
    @action(detail=True, methods=['post'], url_path='run_suite')
    def run_suite(self, request, pk=None):
        """执行 AI 套件：按顺序执行套件内每个 AI 用例，无图形环境自动使用无头模式。"""
        ai_suite = self.get_object()
        suite_cases = list(
            ai_suite.suite_cases.select_related('ai_case').order_by('order')
        )
        if not suite_cases:
            return Response(
                {'error': '该套件未包含任何 AI 用例，无法执行'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        def run_suite_task():
            for sc in suite_cases:
                ...
                try:
                    ...
                    async def on_step_update(step_info):
                        try:
                            if step_info.get('type') == 'log' and step_info.get('content'):
                                execution_record.logs += step_info['content']
                                await sync_to_async(execution_record.save)()
                            elif step_info.get('task_id') and step_info.get('status'):
                                for task in (execution_record.planned_tasks or []):
                                    if str(task.get('id')) == str(step_info['task_id']):
                                        task['status'] = step_info['status']
                                        break
                                await sync_to_async(execution_record.save)(update_fields=['planned_tasks'])
                        except Exception:
                            pass
        ...
        import threading
        t = threading.Thread(target=run_suite_task)
        t.daemon = True
        t.start()
        log_operation('run', 'ai_suite', ai_suite.id, ai_suite.name, request.user)
        return Response({
            'message': 'AI 套件已开始执行',
            'suite_id': ai_suite.id,
...
class AIScheduledTaskViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.OrderingFilter]
    filterset_fields = ['project', 'status', 'task_type']
    ordering = ['-created_at']

    def get_queryset(self):
        return AIScheduledTask.objects.all()
...
class AiNotificationLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.OrderingFilter]
    filterset_fields = ['task', 'status', 'notification_type']
    ordering = ['-created_at']

    def get_queryset(self):
        return AiNotificationLog.objects.all().select_related('task')
...
class UiDashboardViewSet(viewsets.ViewSet):
    ...
    def stats(self, request):
        """获取仪表盘统计数据"""
        accessible_projects = UiProject.objects.all()
