...
class AIIntelligentModeConfigViewSet(viewsets.ViewSet):
    """
    AI智能模式配置视图集 (Browser-use) - 支持文本模式与视觉模式两种配置
    """
    permission_classes = [IsAuthenticated]
    ...
    def list(self, request):
        """
        获取所有AI智能模式配置列表（含文本模式与视觉模式）
        """
        configs = self.queryset.order_by('role', '-created_at')
        serializer_data = [{
            'id': config.id,
            'name': config.name,
            ...
            'role': config.role,  # 'browser_use_text' | 'browser_use_vision'
    ...
    def create(self, request):
        """
        创建新的AI智能模式配置（role 指定文本模式或视觉模式）
        """
        data = request.data
        user = request.user
        role = data.get('role', 'browser_use_text')
        if role not in ('browser_use_text', 'browser_use_vision'):
    ...
    def update(self, request, pk=None):
        ...
        try:
            ...
            if new_is_active:
                active_configs = self.queryset.exclude(pk=pk).filter(role=config.role, is_active=True)
    ...
    @action(detail=False, methods=['get'], url_path='effective_mode')
    def effective_mode(self, request):
        """
        返回当前执行将使用的模式。可选查询参数 for_run: auto|text|vision，表示「若本次选择该模式」时的结果。
        """
        for_run = request.query_params.get('for_run', 'auto').lower()
        if for_run not in ('auto', 'text', 'vision'):
            for_run = 'auto'
        text_config = AIModelConfig.objects.filter(role='browser_use_text', is_active=True).first()
        vision_config = AIModelConfig.objects.filter(role='browser_use_vision', is_active=True).first()
        if for_run == 'vision' and vision_config:
            return Response({
                'effective_mode': 'vision',
                'role': 'browser_use_vision',
                'model_name': vision_config.model_name or '',
            })
        if for_run == 'text' and text_config:
            return Response({
                'effective_mode': 'text',
                'role': 'browser_use_text',
                'model_name': text_config.model_name or '',
            })
        # auto 或未匹配：有文本则文本，否则视觉
        if text_config:
            return Response({
                'effective_mode': 'text',
                'role': 'browser_use_text',
