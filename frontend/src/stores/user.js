...
export const useUserStore = defineStore('user', () => {
  ...
  const login = async (credentials) => {
    try {
      // 将 reactive Proxy 转为纯对象，避免 axios 序列化异常
      const payload = {
  ...
  const logout = async (options = {}) => {
    ...
    try {
      ...
      // 修复：silent 模式下不跳页（用于应用初始化/守卫内 401 清理，避免
      //       window.location.href='/login' 触发的整页刷新死循环）
      if (!silent) {
  ...
  const refreshAccessToken = async () => {
    try {
      ...
      // 刷新失败，清除所有认证信息
      // 修复：silent 模式清除本地状态但不跳 /login，避免 main.js init 阶段死循环
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (error.response?.status === 401) {
        // silent: 仅清状态不跳 /login（在登录页时由拦截器避免再次 reload）
        await logout({ silent: true })
      }
      throw error
    }
    ...
    if (accessToken.value) {
      ...
      if (isTokenExpired.value && refreshToken.value) {
        ...
        try {
          ...
          // silent logout：仅清状态不跳页（避免在 /login 时死循环）
          await logout({ silent: true })
          return
        }
      }
      ...
      if (!user.value) {
        try {
          ...
          // 修复：fetchProfile 已经 silent logout，这里不要再触发 reload
