  ...
  <div class="automation-testing">
        ...
        <div v-else class="suite-detail">
          ...
          <div class="suite-header">
            ...
            <el-alert type="info" :closable="false" show-icon style="margin-top: 8px;">
              <template #title>多环境使用</template>
              同一接口可在多个环境使用：在「接口管理」发送请求或在此运行套件时选择环境即可；环境变量在侧边栏「环境管理」中配置，URL 等可使用双花括号包裹变量名引用（如 base_url）。
            </el-alert>
          </div>
    ...
    >
      <div class="add-request-content">
        ...
        >
          未选择项目时显示全部可访问请求；选择项目后可仅显示该项目下的请求。
        </el-alert>
...
const loadRequestTree = async () => {
  try {
    // 有选项目时：只加载该项目的集合和请求；未选项目时：只加载所有可访问请求（便于按项目添加也能看到）
    const projectId = selectedProject.value || undefined
    const collections = projectId
      ? (await api.get('/api-testing/collections/', { params: { project: projectId } })).data.results || []
      : []

    const requestParams = projectId ? { project: projectId } : {}
    const requestsRes = await api.get('/api-testing/requests/', { params: requestParams })
    const requests = requestsRes.data.results || requestsRes.data || []

    // 构建树形结构（含未分类节点；新保存的未分类请求会出现在「未分类」下）
...
const buildRequestTree = (collections, requests) => {
  ...
  // 未分类节点：用于展示 collection 为空的请求（新保存的未归类接口）
  ...
  requests.forEach(request => {
    if (request.collection && map[request.collection]) {
      map[request.collection].children.push(requestNode(request))
    } else {
      uncategorizedNode.children.push(requestNode(request))
    }
  })
