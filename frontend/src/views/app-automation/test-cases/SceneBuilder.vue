<template>
    <div class="ui-test-scene-builder">
                        ...
                        <div class="card-title">
                            ...
                            <div class="card-title-actions">
                                <el-button
                                    ...
                                    size="small"
                                    :icon="MagicStick"
                                    @click="showAiArrangeDialog = true"
                                >
                                    AI 编排
                                </el-button>
                                <el-button
        ...
        <!-- AI 编排对话框 -->
        <el-dialog
            v-model="showAiArrangeDialog"
            title="AI 编排 — 自然语言生成步骤"
            width="600px"
            :close-on-click-modal="false"
        >
            <el-alert
            ...
            >
                输入测试任务描述，AI 会根据元素库和组件自动生成步骤草稿，生成后可手动调整。
            </el-alert>
            <el-input
                v-model="aiArrangeText"
                type="textarea"
                :rows="6"
                placeholder="例如：打开APP，登录账号 admin/123456，进入设置页面，修改密码为 Abc@5678，然后退出登录"
                :disabled="aiArrangeLoading"
            />
            <template #footer>
                <el-button @click="showAiArrangeDialog = false">取消</el-button>
                <el-button
                    type="primary"
                    :loading="aiArrangeLoading"
                    :disabled="!aiArrangeText.trim()"
                    @click="handleAiArrange"
                >
                    生成步骤
                </el-button>
            </template>
...
// AI 编排
const showAiArrangeDialog = ref(false)
const aiArrangeLoading = ref(false)
const aiArrangeText = ref('')
...
async function handleAiArrange() {
    if (!aiArrangeText.value.trim()) return
    aiArrangeLoading.value = true
    try {
        const projectId = sceneForm.value?.project || route.query.project || null
        const res = await aiArrange({
            natural_language: aiArrangeText.value.trim(),
            project: projectId,
        })
        if (res.data?.success && res.data.steps?.length) {
            // 追加到现有步骤列表
            scenarioSteps.value.push(...res.data.steps)
            ElMessage.success(`AI 生成了 ${res.data.steps.length} 个步骤，请检查并调整`)
            showAiArrangeDialog.value = false
            aiArrangeText.value = ''
        } else {
            ElMessage.warning(res.data?.message || 'AI 未生成有效步骤')
        }
    } catch (err) {
        ElMessage.error('AI 编排请求失败: ' + (err?.response?.data?.message || err.message))
    } finally {
        aiArrangeLoading.value = false
    }
}
