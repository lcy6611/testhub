...
const loginStyle = computed(() => {
  ...
  if (!wp) return {}
  const img = themeStore.isGradient(wp) ? wp : `url("${wp}")`
  return {
    backgroundImage: img,
...
const handleLogin = async () => {
  ...
  // 不使用 async callback 模式，改用 try/catch 确保 validate 完成后再执行登录
  try {
  ...
  try {
    ...
    if (data && typeof data === 'object') {
      ...
      } else if (typeof data.username === 'string') message = data.username
      else if (Array.isArray(data.username) && data.username.length) message = data.username[0]
      else if (typeof data.password === 'string') message = data.password
      else if (Array.isArray(data.password) && data.password.length) message = data.password[0]
    }
...
.login-section {
  ...
  align-items: center;
  justify-content: center;
  background: var(--app-card-bg, rgba(255, 255, 255, 0.82));
  backdrop-filter: blur(var(--app-panel-blur, 12px));
  -webkit-backdrop-filter: blur(var(--app-panel-blur, 12px));
  padding: 60px;
  position: relative;
