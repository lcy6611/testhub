...
import {
  ...
  Phone, Message, Location, Ticket, OfficeBuilding, CreditCard, CircleCheck, DocumentCopy, Search, Delete, Edit, Unlock, DataLine as DataLineIcon, Sort, Share, View, Upload, Download
} from '@element-plus/icons-vue'
import api from '@/utils/api'
import { debounce } from 'lodash-es'
import dataFactoryLocale from './data-factory-locale.js'

function t(key, opts) {
  const k = key.replace(/^dataFactory\./, '')
  const parts = k.split('.')
  let v = dataFactoryLocale
...
const downloadImage = (result) => {
  ...
  const downloadUrl = `/data-factory/download_static_file/?filename=${filename}`
