...
import { createDefect, getDefect, updateDefect, uploadDefectAttachment } from '@/api/defects_oss'
import {
  defectTypeOptions,
  priorityOptions,
  severityOptions,
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      router.push(`/defects_oss/${route.params.id}`)
    } else {
      ...
      router.push(`/defects_oss/${defectId || 'list'}`)
    }
  } catch (error) {
    if (error?.response) {
