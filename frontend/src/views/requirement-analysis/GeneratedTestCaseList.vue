    ...
    <div class="filters-section">
      <div class="filter-card">
        <div class="filter-group">
          <label>状态筛选:</label>
          <select v-model="selectedStatus" @change="onStatusFilterChange" class="filter-select">
            ...
            <div class="body-cell">
              <div class="action-buttons">
                ...
                <button
                  ...
                  class="adopt-btn"
                  :disabled="task.is_saved_to_records"
                  @click.stop="oneKeyAdopt(task)">
                  ✅ 一键采纳
                </button>
                <button
                  v-if="task.status === 'completed'"
                  class="discard-btn"
                  @click.stop="oneKeyDiscard(task)">
                  ❌ 一键弃用
                </button>
              </div>
            </div>
  ...
  methods: {
    async loadTasks() {
      ...
      try {
        ...
        // DRF 请求的页码超出总页数时返回 404（中文环境 detail 多为「无效页面」）
        const status = error?.response?.status
        const detail = error?.response?.data?.detail
        if (status === 404 && this.pagination.currentPage > 1) {
        ...
        if (detail) {
          ElMessage.error(typeof detail === 'string' ? detail : '列表加载失败')
        }
      } finally {
    ...
    viewTaskDetail(task) {
      ...
      if (task.status === 'completed') {
        ...
        const url = this.$router.resolve({
          name: 'TaskDetail',
          // 后端详情接口按主键 id（数字）查询，task_id 仅用于展示
    ...
    async oneKeyAdopt(task) {
      if (!task || task.status !== 'completed') return
      if (task.is_saved_to_records) {
        ElMessage.info('该任务已采纳，无需重复操作')
        return
      }
      if (!confirm(`确定要一键采纳任务 "${task.title}" 的用例吗？将导入到“测试用例列表”。`)) {
        return
      }
      try {
        const resp = await api.post(`/requirement-analysis/testcase-generation/${task.id}/save_to_records/`)
        const created = resp?.data?.created_count ?? resp?.data?.count ?? 0
        ElMessage.success(`一键采纳成功${created ? `（导入 ${created} 条）` : ''}`)
        this.loadTasks()
      } catch (error) {
        console.error('一键采纳失败:', error)
        ElMessage.error('一键采纳失败：' + ((error?.response?.data?.detail) || error.message || '未知错误'))
      }
    },

    async oneKeyDiscard(task) {
      if (!task || task.status !== 'completed') return
      if (!confirm(`确定要一键弃用任务 "${task.title}" 的用例吗？此操作不可恢复。`)) {
        return
      }
      try {
        await api.post(`/requirement-analysis/testcase-generation/${task.id}/discard/`)
        ElMessage.success('一键弃用成功')
        this.loadTasks()
      } catch (error) {
        console.error('一键弃用失败:', error)
        ElMessage.error('一键弃用失败：' + ((error?.response?.data?.detail) || error.message || '未知错误'))
      }
    },
