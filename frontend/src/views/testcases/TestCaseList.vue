  ...
  <div class="page-container">
    <div class="page-header">
      ...
      <div class="header-actions">
        <el-button
          ...
          type="warning"
          @click="onBatchGenerateUiScript"
          :disabled="isGeneratingUi">
          <el-icon><MagicStick /></el-icon>
          生成UI脚本 ({{ selectedTestCases.length }})
        </el-button>
        <span v-if="detectedUiProject" class="gen-hint" :class="detectedUiProject.needsDialog ? 'warn' : 'ok'">
          <template v-if="detectedUiProject.needsDialog">
            {{ detectedUiProject.multi ? '⚠ 所选用例关联多个 UI 项目，将弹窗选择' : '⚠ 所选用例未关联 UI 项目，将弹窗输入测试地址' }}
          </template>
          <template v-else>→ 将生成到「{{ detectedUiProject.project.name }}」</template>
        </span>
        <el-button 
          v-if="selectedTestCases.length > 0" 
          type="danger" 
    ...
    <div class="card-container">
      ...
      <div class="pagination-container">
        ...
        />
      <!-- 批量生成UI脚本 弹窗（仅在选中用例未关联UiProject时打开） -->
    <el-dialog v-model="genDialog.visible" title="生成UI脚本" width="540px" :close-on-click-modal="false">
      <el-alert type="info" :closable="false" style="margin-bottom:12px">
        已选 <b>{{ selectedTestCases.length }}</b> 条用例，但所关联业务项目下未找到配置了「测试地址」的 UI 项目。请选择已有 UI 项目，或临时输入测试地址（系统会自动建一个临时 UI 项目）。
      </el-alert>
      <el-form label-width="100px">
        <el-form-item label="UI 项目">
          <el-select v-model="genDialog.ui_project_id" filterable placeholder="选择已有 UI 项目（关联/全部）" style="width:100%" clearable>
            <el-option-group v-for="g in groupedProjects" :key="g.label" :label="g.label">
              <el-option v-for="p in g.items" :key="p.id"
                         :label="`${p.name}（${p.base_url || '无地址'}）`" :value="p.id" />
            </el-option-group>
          </el-select>
        </el-form-item>
        <el-form-item label="测试地址">
          <el-input v-model="genDialog.base_url" placeholder="https://example.com" />
          <div style="color:#999;font-size:12px;margin-top:4px">未选 UI 项目时填此项；填了地址则系统自动按选中用例所属业务项目建临时 UI 项目</div>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="genDialog.visible = false">取消</el-button>
        <el-button type="primary" :loading="isGeneratingUi" @click="confirmGenerateUi">确认生成</el-button>
      </template>
    </el-dialog>
      </div>
    </div>
...
import { Plus, Search, Download, Delete, MagicStick } from '@element-plus/icons-vue'
import api from '@/utils/api'
import dayjs from 'dayjs'
import * as XLSX from 'xlsx'
import { createCaseScriptGeneration, getUiProjects } from '@/api/ui_automation'
...
const isGeneratingUi = ref(false)
const genDialog = ref({ visible: false, ui_project_id: null, base_url: '' })
const groupedProjects = computed(() => {
  const linked = []
  const free = []
  uiProjects.value.forEach(p => {
    const item = { id: p.id, name: p.name, base_url: p.base_url }
    if (p.project?.id) linked.push({ ...item, bizId: p.project.id, bizName: p.project.name })
    else free.push(item)
  })
  // 按业务项目分组（同名聚合）
  const map = new Map()
  linked.forEach(it => {
    const key = `${it.bizId}__${it.bizName}`
    if (!map.has(key)) map.set(key, { label: `关联业务项目：${it.bizName}`, items: [] })
    map.get(key).items.push(it)
  })
  const groups = Array.from(map.values())
  if (free.length) groups.push({ label: '未关联业务项目', items: free })
  return groups
})
...
const fetchUiProjects = async () => {
  try {
    ...
    uiProjects.value = r.data.results || r.data || []
  } catch (e) { /* ignore */ }
}

// 在已加载的 uiProjects 中找「选中第一条 tc 所属业务项目下有 base_url 的 UiProject」
function findLinkedUiProject() {
  if (!selectedTestCases.value.length) return null
  const firstBizId = selectedTestCases.value[0].project?.id
  if (!firstBizId) return null
  const candidates = uiProjects.value.filter(p => p.project?.id === firstBizId && p.base_url)
  return candidates.length === 1 ? candidates[0] : null
}

// 选中用例时实时识别目标 UiProject：显示"将生成到 XX"或"需弹窗选择"
const detectedUiProject = computed(() => {
  const sels = selectedTestCases.value
  if (!sels.length) return null
  const found = sels.map(tc => {
    const bizId = tc.project?.id
    if (!bizId) return null
    const cands = uiProjects.value.filter(p => p.project?.id === bizId && p.base_url)
    return cands.length === 1 ? cands[0] : null
  }).filter(Boolean)
  if (!found.length) return { needsDialog: true, multi: false }
  const firstId = found[0].id
  const allSame = found.every(p => p.id === firstId)
  if (allSame) return { needsDialog: false, project: found[0] }
  return { needsDialog: true, multi: true }
})

const onBatchGenerateUiScript = async () => {
  if (!selectedTestCases.value.length) {
    ElMessage.warning('请先勾选用例')
    return
  }
  isGeneratingUi.value = true
  try {
    if (!uiProjects.value.length) await fetchUiProjects()
    const linked = findLinkedUiProject()
    if (linked) {
      // 已关联有 base_url → 直接生成，不弹窗
      await doGenerate(linked.id, linked.base_url)
    } else {
      // 未关联或多个 → 弹窗
      genDialog.value = { visible: true, ui_project_id: null, base_url: '' }
    }
  } finally {
    isGeneratingUi.value = false
  }
}

const confirmGenerateUi = async () => {
  if (!genDialog.value.ui_project_id && !genDialog.value.base_url) {
    ElMessage.warning('请选择 UI 项目 或 填入测试地址')
    return
  }
  if (genDialog.value.ui_project_id) {
    const p = uiProjects.value.find(x => x.id === genDialog.value.ui_project_id)
    await doGenerate(p.id, p.base_url)
  } else {
    await doGenerate(null, genDialog.value.base_url)
  }
  genDialog.value.visible = false
}

const doGenerate = async (uiProjectId, baseUrl) => {
  if (!baseUrl) {
    ElMessage.warning('所选 UI 项目未配置测试地址')
    return
  }
  isGeneratingUi.value = true
  try {
    const ids = selectedTestCases.value.map(t => t.id)
    const r = await createCaseScriptGeneration({
      source_testcase_ids: ids,
      ui_project_id: uiProjectId,
      base_url: baseUrl,
    })
    const arr = r.data || []
    ElMessage.success(`已创建 ${arr.length} 条 UI 脚本生成任务，前往「UI 自动化 → 用例生成UI脚本」查看进度`)
    selectedTestCases.value = []
  } catch (e) {
...
onMounted(() => {
  fetchProjects()
  fetchUiProjects()  // 预拉取 UI 项目，避免首次点按钮时等待
  fetchTestCases()
})
</script>
...
.gen-hint {
  ...
  padding: 2px 8px;
  border-radius: 4px;
  &.ok { color: #67c23a; background: rgba(103,194,58,0.12); }
