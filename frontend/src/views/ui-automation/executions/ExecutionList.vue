  ...
  <div class="page-container">
    ...
    <div class="card-container">
      <div class="filter-bar">
        <el-form :inline="true" :model="queryParams" class="demo-form-inline">
          ...
          <el-form-item label="状态">
            <el-select v-model="queryParams.status" placeholder="执行状态" clearable style="width: 160px">
          ...
          <el-form-item label="浏览器">
            <el-select v-model="queryParams.browser" placeholder="浏览器" clearable style="width: 160px">
      ...
      <el-table :data="executions" v-loading="loading" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="test_case_name" label="用例/脚本名称" min-width="220">
        ...
        <el-table-column label="关联对象" width="160" align="center">
          <template #default="{ row }">
            <template v-if="row.test_suite">
              <el-tag type="warning" size="small">
                套件（{{ row.test_suite_name || row.test_suite.name || '未知套件' }}）
              </el-tag>
            </template>
            <template v-else>
              <el-tag type="info" size="small">用例</el-tag>
            </template>
          </template>
        </el-table-column>
        <el-table-column prop="status" label="执行状态" width="110" align="center" show-overflow-tooltip>
          <template #default="{ row }">
        ...
        <el-table-column prop="browser" label="浏览器" width="90" align="center" show-overflow-tooltip>
          <template #default="{ row }">
            {{ getBrowserText(row.browser) }}
          </template>
        </el-table-column>
        <el-table-column prop="executed_by_name" label="执行人" width="120" align="center">
          <template #default="{ row }">
            {{ row.executed_by_name ?? row.created_by_name ?? '-' }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="执行时长" width="120" align="center">
          <template #default="{ row }">
            {{ formatDuration(row.execution_time ?? row.duration) }}
          </template>
        </el-table-column>
        <el-table-column label="操作" width="150" fixed="right" align="center">
          <template #default="{ row }">
            ...
            <el-button
              v-if="['failed', 'error', 'FAILED'].includes(row.status) && getRerunTestCaseId(row)"
              size="small"
              type="warning"
              link
    ...
    <el-dialog v-model="showDetailDialog" title="执行详情" width="900px">
      <div v-if="currentExecution" class="execution-detail">
        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="执行人">{{ currentExecution.executed_by_name ?? currentExecution.created_by_name ?? '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="执行时长" :span="2">{{ formatDuration(currentExecution.execution_time ?? currentExecution.duration) }}</el-descriptions-item>
        </el-descriptions>
        ...
        <el-tabs v-model="activeTab" class="execution-tabs" style="margin-top: 20px;">
          ...
          <el-tab-pane label="执行日志" name="logs">
            <div class="logs-container">
              <div v-if="getExecutionLogs(currentExecution).length">
                <div v-for="(step, index) in getExecutionLogs(currentExecution)" :key="index" class="log-item">
          ...
          <!-- 失败截图 - 仅失败或错误状态显示 -->
          <el-tab-pane label="失败截图" name="screenshots" v-if="['failed', 'error', 'FAILED'].includes(currentExecution.status)">
            <div class="screenshots-container">
              <div v-if="getExecutionScreenshots(currentExecution).length">
                <div v-for="(screenshot, index) in getExecutionScreenshots(currentExecution)" :key="index" class="screenshot-item">
          ...
          <!-- 错误信息 - 仅失败或错误状态显示 -->
          <el-tab-pane label="错误信息" name="error" v-if="['failed', 'error', 'FAILED'].includes(currentExecution.status)">
...
// 获取状态样式（兼容 TestExecution 的 SUCCESS/FAILED 与 TestCaseExecution 的 passed/failed）
...
// 获取执行日志（TestExecution 用 result_data.test_cases[0].steps，TestCaseExecution 用 execution_logs）
const getExecutionLogs = (execution) => {
  if (!execution) return []
  if (execution.execution_logs) return parseExecutionLogs(execution.execution_logs)
  const rd = execution.result_data || {}
  const cases = rd.test_cases || []
  const first = cases[0]
  if (first && first.steps && first.steps.length) return first.steps
  return []
}

// 获取执行截图（TestExecution 用 result_data.test_cases[0].screenshots）
const getExecutionScreenshots = (execution) => {
  if (!execution) return []
  if (execution.screenshots && execution.screenshots.length) return execution.screenshots
  const rd = execution.result_data || {}
  const cases = rd.test_cases || []
  const first = cases[0]
  if (first && first.screenshots && first.screenshots.length) return first.screenshots
  return []
}

// 仅用例执行可重跑：TestExecution 从 result_data.test_cases[0].test_case_id 取用例 id
const getRerunTestCaseId = (execution) => {
  if (execution.test_case) return execution.test_case
  if (execution.test_script) return null
  const rd = execution.result_data || {}
  const cases = rd.test_cases || []
  const first = cases[0]
  return (first && first.test_case_id) || null
}
...
const showRerunDialog = (execution) => {
  rerunFormData.testCaseId = getRerunTestCaseId(execution)
