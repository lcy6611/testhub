...
export default {
  ...
  setup() {
    ...
    const getNotificationTypeTagType = (typeDisplay) => {
      // 参照接口自动化：邮箱通知也有背景色；并区分飞书/企微/钉钉
      if (!typeDisplay) return 'info'
      // 避免与“发送成功”状态的绿色撞色
      if (typeDisplay.includes('邮箱')) return 'info'
      if (typeDisplay.includes('飞书')) return 'primary'
      if (typeDisplay.includes('企微')) return 'success'
      if (typeDisplay.includes('钉钉')) return 'warning'
      if (typeDisplay.includes('机器人')) return 'primary'
      if (typeDisplay.includes('两种')) return 'warning'
      return 'info'
    }

    // 通知类型标签自定义样式（邮箱通知用深黄色背景，不与“发送成功”撞色）
    const getNotificationTypeTagStyle = (typeDisplay) => {
      if (!typeDisplay) return {}
      if (typeDisplay.includes('邮箱')) {
        return {
          // 与 Webhook 机器人保持一致：light 风格（浅底 + 有色字 + 有色边框），仅颜色区分
          backgroundColor: '#FFF7E6', // 更接近 light 风格的浅黄底
          borderColor: '#FFE4B3',     // 边框颜色调浅，避免显得“过深”
          color: '#B88200'            // 字体保持偏深黄，确保可读性
        }
      }
      return {}
    }

    // 格式化Webhook机器人显示（区分飞书/企微/钉钉，兼容type/bot_type、name/bot_name）
    const formatWebhookBotLabel = (bot) => {
      if (!bot) return 'Webhook机器人'
      const botType = bot.type || bot.bot_type || ''
      const botName = bot.name || bot.bot_name || ''
      const typeMap = {
        wechat: '企微机器人',
        feishu: '飞书机器人',
        dingtalk: '钉钉机器人'
      }
      const botDisplay = typeMap[botType] || 'Webhook机器人'
      return botName ? `${botDisplay}（${botName}）` : botDisplay
    }
    ...
    return {
      ...
      getNotificationTypeTagStyle,
      formatWebhookBotLabel
    }
  }
}
</script>
